...
import { getCurrentUser, getAttendanceRecords, calculateAttendancePercentage } from '../../data/demoData';
...
export default function Attendance() {
  ...
  useEffect(() => {
    if (!authUser) return;
    const data = getAttendanceRecords(authUser.id);
    setRecords(data);
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800">My Attendance Record</h1>
          <p className="text-xs text-slate-500 mt-0.5">Track your daily presence and overall class attendance.</p>
        </div>
        ...
        <div className="px-5 py-3 rounded-2xl bg-emerald-50 border border-emerald-200 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-600 font-medium">
              {percentage >= 75 ? 'Good Standing' : 'Needs Improvement'}
            </p>
          </div>
        </div>
      </div>
