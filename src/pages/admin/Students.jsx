...
import { getStudents, getParentForStudent, updateStudent, deleteStudent } from '../../data/demoData';
            ...
            <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
              {filteredStudents.length === 0 ? (
                ...
                filteredStudents.map((student) => {
                  const parentInfo = getParentForStudent(student.id);
                  return (
            ...
            <div className="space-y-3 text-sm">
              <div className="grid grid-cols-2 gap-3 p-3 bg-slate-50 rounded-xl">
                ...
                <div>
                  ...
                  <p className="font-semibold text-slate-700">{viewingStudent.className}</p>
                </div>
                <div>
              ...
              {/* Connected Parent Card inside Modal */}
              {(() => {
                const parent = getParentForStudent(viewingStudent.id);
                return (
                  <div className="p-4 rounded-2xl border border-emerald-200/80 bg-emerald-50/40 space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-xs font-bold uppercase tracking-wider text-emerald-900">Connected Parent</p>
                      </div>
                      {parent && (
                    ...
                    {parent ? (
                      <div className="grid grid-cols-2 gap-3 text-xs bg-white p-3.5 rounded-xl border border-emerald-100 shadow-2xs">
                        <div>
                          <p className="text-[11px] text-slate-400 font-medium">Parent Name</p>
                        ...
                        <div>
                          ...
                          <p className="font-medium text-slate-700">{parent.email}</p>
                        </div>
                        <div>
                        ...
                        <div className="col-span-2 pt-2 border-t border-slate-100 flex items-center justify-between">
                          <div>
                            <p className="text-[11px] text-slate-400 font-medium">Parent Password</p>
                            <p className="font-semibold font-mono text-emerald-700 text-sm tracking-wider">
                              {showParentPass ? (parent.password || 'parent123') : '••••••••'}
                            </p>
                          </div>
                          <div className="flex items-center gap-1">
                            <button
                              ...
                              title={showParentPass ? "Hide password" : "Show password"}
                            >
                            ...
                            <button
                              type="button"
                              onClick={() => {
                                navigator.clipboard.writeText(parent.password || 'parent123');
                                setCopiedField('parent');
                                setTimeout(() => setCopiedField(null), 2000);
                              }}
                              className="p-1.5 rounded-lg text-slate-400 hover:text-emerald-700 hover:bg-emerald-50 transition-colors flex items-center gap-1 text-xs font-semibold"
                              title="Copy parent password"
                            >
                              {copiedField === 'parent' ? <Check className="w-4 h-4 text-emerald-700" /> : <Copy className="w-4 h-4" />}
                              {copiedField === 'parent' && <span className="text-emerald-700 text-[11px]">Copied!</span>}
                            </button>
                          </div>
                        </div>
                      </div>
